...
function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // টাস্ক ক্রিয়েট হওয়ার পর সাথে সাথে ডাটাবেস থেকে নতুন লিস্ট আনবে
        fetchTodos(); 
  ...
  // ডাটাবেস থেকে আসা ইমেজের পাথ সার্ভার লিংকে কনভার্ট করা
  const getImageUrl = (path) => {
    if (!path) return null;
    const normalizedPath = path.replace(/\\/g, '/'); // উইন্ডোজ ব্যাকস্ল্যাশ ফিক্স
    ...
    <>
      <div className="min-h-screen bg-[#0f172a] bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(120,119,198,0.3),rgba(255,255,255,0))] text-white flex items-center justify-center p-4 md:p-10 font-sans">
        <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 bg-white/[0.02] backdrop-blur-2xl border border-white/10 rounded-[2rem] p-8 shadow-2xl flex flex-col gap-6 relative overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
              ...
              <button 
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-indigo-500 to-cyan-500 hover:from-indigo-600 hover:to-cyan-600 disabled:opacity-50 text-white font-bold py-4 rounded-2xl shadow-[0_0_20px_rgba(99,102,241,0.3)] transition-all active:scale-[0.98] flex items-center justify-center gap-2 mt-2">
          ...
          <div className="md:col-span-2 bg-white/[0.02] backdrop-blur-2xl border border-white/10 rounded-[2rem] p-8 shadow-2xl flex flex-col h-[700px]">
            ...
            <div className="flex flex-col gap-4 overflow-y-auto pr-2 pb-4 scrollbar-hide flex-1">
              ...
              {todos.map((item) => (
                <div key={item._id} className="group bg-black/20 border border-white/5 hover:border-white/20 rounded-2xl p-4 md:p-5 flex gap-4 md:gap-5 items-center transition-all hover:bg-white/[0.02]">
